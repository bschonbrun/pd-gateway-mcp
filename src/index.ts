...
server.tool(
  'list_templates',
  'List all available flow templates with their parameters and supported channels.',
  {},
  async () => {
    try {
...
server.tool(
  'run_template',
  'Execute a flow template by ID. Resolves parameters from template defaults + optional overrides, then runs the flow (data fetch → format → deliver). Use list_templates first to see available templates and their parameters.',
  {
    template_id: z.string().describe('Template ID (e.g. "daily-revenue-report")'),
    dry_run: z.boolean().default(false).describe('If true, returns formatted output without sending'),
    overrides: z.record(z.string(), z.unknown()).optional().describe('Override template parameter defaults (e.g. {"channels": ["slack"], "email_recipients": ["new@example.com"]})'),
  },
  async ({ template_id, dry_run, overrides }) => {
    try {
...
server.tool(
  ...
  async ({ template_id, webhook_url, overrides }) => {
    try {
      const result = await deployTemplateWorkflow(
        template_id,
        rest,
        env('PIPEDREAM_PROJECT_ID'),
        overrides ?? {},
...
async function main() {
  ...
  console.error('pd-gateway-mcp: running (23 tools)');
}
